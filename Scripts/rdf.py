# RDF calculation using MISPR analysis workflow
...
solv_output_files = solv_df["name"].tolist()  # List of all solvent systems
...
for solv_name in solv_output_files:
    ...
    # Read FW.json for molecule/atom counts and masses
    ...
    # RDF parameters
    r_cut = 20
    bin_size = [0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1, 2]
    ...
    analysis_settings = [{
        ...
        "path_or_buff": "rdf_co2_com_molecular.csv"
    }]
    ...
    wf = Workflow(fws, links_dict, name=f"rdf_{solv_name}")
    lpad.add_wf(wf)
